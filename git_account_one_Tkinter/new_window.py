...
class Windows:
    ...
    def create_window(self):
        ...
        product_field.grid(row=1, column=2,sticky = self.tk.W)

        product_command = self.tk.Label(window_two, text="Produkt")
        product_command.grid(row=1, column=1,sticky = self.tk.E)

        price_field = self.tk.Entry(window_two)
        price_field.grid(row=2, column=2,sticky = self.tk.W)

        price_command = self.tk.Label(window_two, text="Cena")
        price_command.grid(row=2, column=1,sticky = self.tk.E)
        ...
        # Etykieta na podliczenie lokatorów
        elements_etiquette.window_for_settlement()
